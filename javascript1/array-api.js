...
{
    const a = students.find((students) => students.score == 90);
    console.log(a);
}

// Q6. make an array of enrolled students (학원에 등록한 학생들만 나열하는 것, true인 학생들만 찾기!)
{
    const a = students.filter((students) => students.enrolled);
    console.log(a);
}
...
{
    const a = students.map((students) => students.score);
    console.log(a);
}

// Q8. check if there is a student with the score lower than 50 (학생들 중에 50점보다 낮은 점수가 있는지 없는지 확인하라.)
{
    const a = students.some((students) => students.score < 50);
    console.log(a);
    
    const b = students.every((students) => students.enrolled.score >= 50);
    console.log(a);
}
