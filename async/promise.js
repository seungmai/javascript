...
const promise = new Promise((resolve, reject) => { //Promise라는 object를 만듬. resolve: 기능을 정상적으로 수행해서 마지막에 최종 데이터를 전달하는 콜백함수, reject: 기능을 수행하다가 중간에 문제가 생기면 호출하게될 콜백함수
  // doing some heavy work (network, read files), 보통 promise안에서 어려운 작업들을 한다. 왜냐하면 우리가 네트워크에서 데아터를 받아오거나 파일에서 무언가 큰 데이터를 읽어오는 과정은 시간이 꽤 걸린다.그런 것을 동기적으로 처리하게 되면 우리가 파일을 읽어오고 네트워크에서 데이터를 받아오는 동안 그 다음라인에 코드가 실행되지 않기 때문에 시간이 조금 걸리는 일들은 promise를 만들어서 비동기적으로 처리하는 것이 좋다. 그래서 네크워크 통신하던지 파일을 읽어서 온다던
